...
export default function AdminUsersPage() {
  ...
  async function loadUsers() {
    ...
    try {
      ...
      const nextMessage = error instanceof Error ? error.message : "Cannot load users";
  ...
  async function onChangeRole(userId: number, role: UserRole) {
    ...
    try {
      ...
      setItems((prev) => prev.map((item) => (item.id === userId ? data.item : item)));
      setMessage(`Updated role for user #${userId} to ${role}`);
    } catch (error) {
      const nextMessage = error instanceof Error ? error.message : "Update role failed";
      setMessage(nextMessage);
    } finally {
      setUpdatingId(null);
    }
  ...
  if (!auth) {
    return <p className="rounded-2xl bg-white p-4 shadow">Please login as ADMIN to manage users.</p>;
  }
  ...
  return (
    <section className="space-y-4 rounded-3xl bg-white p-6 shadow-lg">
      ...
      <p className="text-sm text-slate-600">Update user roles directly from web UI.</p>
      <button
        type="button"
        onClick={loadUsers}
        disabled={isLoading}
              ...
              <tr key={item.id}>
                ...
                <td className="py-2 pr-4">
                  <select
                    ...
                    onChange={(event) => onChangeRole(item.id, event.target.value as UserRole)}
                  >
                    {ROLES.map((role) => (
